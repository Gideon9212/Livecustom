--Elemelon Tiller
--AlphaKretin
function c515310011.initial_effect(c)
	...
	e3:SetOperation(c515310011.counter)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	...
	e6:SetRange(LOCATION_SZONE)
	e6:SetOperation(c515310011.ctop)
	c:RegisterEffect(e6)
	--move
	...
	e7:SetCost(c515310011.seqcost)
	e7:SetTarget(c515310011.seqtg)
	e7:SetOperation(c515310011.seqop)
	c:RegisterEffect(e7)
	--return
	...
	e8:SetCost(c515310011.rtgcost)
	e8:SetTarget(c515310011.rtgtg)
	e8:SetOperation(c515310011.rtgop)
	c:RegisterEffect(e8)
end
function c515310011.valcon(e,re,r,rp)
	return bit.band(r,REASON_EFFECT)~=0
end
function c515310011.cfilter(c)
	return c:IsSetCard(0xf31)
end
function c515310011.counter(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsExists(c515310011.cfilter,1,nil) then
		e:GetHandler():AddCounter(0x20,1)
	end
end
function c515310011.ctop(e,tp,eg,ep,ev,re,r,rp)
	if re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsSetCard(0xf31) and e:GetHandler():GetFlagEffect(1)>0 then
		e:GetHandler():AddCounter(0x20,1)
	end
end
function c515310011.seqcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c515310011.seqfilter(c,tp)
...
function c515310011.rtgcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c515310011.rtgfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xf31) and c:IsType(TYPE_MONSTER)
end
function c515310011.rtgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c515310011.rtgfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c515310011.rtgfilter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c515310011.rtgfilter,tp,LOCATION_REMOVED,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,g,1,0,0)
end
function c515310011.rtgop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
