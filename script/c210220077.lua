...
function s.filter(c,e,tp,m,m2,ft)
	...
	if ft>0 then
		res=mg:CheckWithSumEqual(Card.GetRitualLevel,c:GetOriginalLevel()*2,2,99,c)
	else
...
function s.filterf(c,tp,mg,rc)
	if c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:GetSequence()<5 then
		...
		return mg:CheckWithSumEqual(Card.GetRitualLevel,rc:GetOriginalLevel()*2,1,99,rc)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_DECK,0,1,1,nil,filter,e,tp,mg,mg2,ft)
