...
function cardID.initial_effect(c)
	...
	e1:SetCountLimit(1,210777045)
	e1:SetTarget(cardID.sptg)
	e1:SetOperation(cardID.spop)
	c:RegisterEffect(e1)
	--spsummon LV 5
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(210777045,1))
...
function cardID.spfilter(c,e,tp)
	return c:IsLevel(5) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function cardID.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(cardID.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
end
function cardID.spop2(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(cardID.spfilter),tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
