...
function c515958933.initial_effect(c)
	...
	e1:SetCondition(c515958933.penconn)
	e1:SetTarget(c515958933.pentgg)
	e1:SetOperation(c515958933.penopp)
	c:RegisterEffect(e1)
	...
	e2:SetCondition(c515958933.pensetcon)
	e2:SetTarget(c515958933.pensettg)
	e2:SetOperation(c515958933.pensetop)
	c:RegisterEffect(e2)	
	--atkup
...
--Destruction hand and scale
function c515958933.penconn(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c515958933.penf1,tp,LOCATION_ONFIELD,0,1,nil)
end
function c515958933.penff1(c)
	return c:IsOnField()
end
function c515958933.penff2(c,self)
    if c:IsCode(c,515958933) and c:GetFieldID()==self then 
    return c end
end
function c515958933.pentgg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	local self=e:GetHandler():GetFieldID()
	if chk==0 then return Duel.IsExistingTarget(c515958933.penff2,tp,LOCATION_HAND,0,1,nil,self) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,c515958933.penff2,tp,LOCATION_HAND,0,1,1,nil,self)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g2=Duel.SelectTarget(tp,c515958933.penff1,tp,LOCATION_ONFIELD,0,1,1,nil)
	g1:Merge(g2)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g1,2,0,0)
end
function c515958933.penopp(e,tp,eg,ep,ev,re,r,rp)
