...
function c210310305.cfilter1(c,tp)
   return c:GetPreviousControler()==tp and c:IsPreviousLocation(LOCATION_MZONE) and bit.band(c:GetPreviousRaceOnField(),RACE_FAIRY)==0 and c:IsArchetype(Archetype_0x18) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c210310305.spfilter1(c,e,tp)
   return  c:IsArchetype(Archetype_0x18) and c:IsRace(RACE_AQUA) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function c210310305.cfilter2(c,tp)
   return c:GetPreviousControler()==tp and c:IsPreviousLocation(LOCATION_MZONE) and bit.band(c:GetPreviousRaceOnField(),RACE_AQUA)==0 and c:IsArchetype(Archetype_0x18) and c:IsPreviousPosition(POS_FACEUP)
end
...
function c210310305.spfilter2(c,e,tp)
   return  c:IsArchetype(Archetype_0x18) and c:IsRace(RACE_FAIRY) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
