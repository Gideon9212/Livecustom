...
--2017 edo9300
function c515004000.initial_effect(c)
	...
	lp:SetCondition(c515004000.lpcon)
	lp:SetOperation(c515004000.lpop)
	c:RegisterEffect(lp)
	--Damage LV/RK
	local dam=lp:Clone()
	dam:SetCondition(c515004000.dmcon)
	dam:SetOperation(c515004000.dmop)
	c:RegisterEffect(dam)
	--Gain Xyz's ATK/DEF
	local ad=lp:Clone()
	ad:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	ad:SetCondition(c515004000.adcon)
	ad:SetOperation(c515004000.adop)
	c:RegisterEffect(ad)
	--Return/pzone
	...
	ret:SetCountLimit(1)
	ret:SetCondition(c515004000.retcon)
	ret:SetTarget(c515004000.rettg)
	ret:SetOperation(c515004000.retop)
	c:RegisterEffect(ret)
	local mov=ret:Clone()
	mov:SetCondition(c515004000.pencon)
	mov:SetTarget(c515004000.pentg)
	mov:SetOperation(c515004000.penop)
	c:RegisterEffect(mov)
	--Win
	local win=Effect.CreateEffect(c)
	win:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	win:SetCode(EVENT_CUSTOM+515004000)
	win:SetRange(LOCATION_PZONE)
	win:SetTarget(c515004000.wtg)
	win:SetOperation(c515004000.wop)
	c:RegisterEffect(win)
	if c515004000.global_check==nil then
		c515004000.global_check=true
		for i=0,1 do
			c515004000[i]={}
			c515004000[i][SUMMON_TYPE_FUSION]=false
			c515004000[i][SUMMON_TYPE_SYNCHRO]=false
			c515004000[i][SUMMON_TYPE_XYZ]=false
		end
		local ge1=Effect.CreateEffect(c)
		ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge1:SetCode(EVENT_SPSUMMON_SUCCESS)
		ge1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		ge1:SetOperation(c515004000.sumop)
...
function c515004000.xyzcheck(g,tp,xyz)
    return g:IsExists(c515004000.xyzchk,1,nil,xyz,SUMMON_TYPE_XYZ,tp)
end
function c515004000.xyzchk(c,g,tp,xyz)
    return c:IsType(TYPE_TUNER,xyz,SUMMON_TYPE_XYZ,tp) and 
        g:IsExists(aux.NOT(Card.IsType),1,c,TYPE_TUNER,xyz,SUMMON_TYPE_XYZ,tp)
end
	...
	for tc in aux.Next(eg) do
		if tc:GetOriginalCode()==e:GetHandler():GetOriginalCode() then 
			if tc:IsSummonType(SUMMON_TYPE_FUSION) then
				c515004000[tc:GetSummonPlayer()][SUMMON_TYPE_FUSION]=true
			end
			if tc:IsSummonType(SUMMON_TYPE_SYNCHRO) then
				c515004000[tc:GetSummonPlayer()][SUMMON_TYPE_SYNCHRO]=true
			end
			if tc:IsSummonType(SUMMON_TYPE_XYZ) then
				c515004000[tc:GetSummonPlayer()][SUMMON_TYPE_XYZ]=true
			end
		end
	end
end
...
function c515004000.lpcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION)
end
function c515004000.lpop(e,tp,eg,ep,ev,re,r,rp)
...
function c515004000.adcost(e,tp,eg,ep,ev,re,r,rp)
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) and Duel.IsExistingMatchingCard(c515004000.adfilter,e:GetHandlerPlayer(),LOCATION_EXTRA,0,1,nil) end
	local g=e:GetHandler():GetOverlayGroup()
	Duel.SendtoGrave(g,REASON_COST)
end
function c515004000.adcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_XYZ)
end
function c515004000.adop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=c:GetOverlayGroup()
	if Duel.SendtoGrave(g,REASON_COST)~=0 and Duel.IsExistingMatchingCard(c515004000.adfilter,e:GetHandlerPlayer(),LOCATION_EXTRA,0,1,nil) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
		local cg=Duel.SelectMatchingCard(tp,c515004000.adfilter,e:GetHandlerPlayer(),LOCATION_EXTRA,0,1,1,nil)
...
function c515004000.retcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c515004000[c:GetOwner()][SUMMON_TYPE_FUSION]==false or c515004000[c:GetOwner()][SUMMON_TYPE_SYNCHRO]==false or c515004000[c:GetOwner()][SUMMON_TYPE_XYZ]==false
end
function c515004000.rettg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToExtra() end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,e:GetHandler(),1,0,0)
end
function c515004000.retop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) or c:IsFacedown() then return end
	Duel.SendtoDeck(c,nil,2,REASON_EFFECT)
end
function c515004000.pencon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c515004000[c:GetOwner()][SUMMON_TYPE_FUSION]==true and c515004000[c:GetOwner()][SUMMON_TYPE_SYNCHRO]==true and c515004000[c:GetOwner()][SUMMON_TYPE_XYZ]==true
end
function c515004000.pentg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c515004000.penop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	if c:IsRelateToEffect(e) then
		if Duel.MoveToField(c,c:GetOwner(),c:GetOwner(),LOCATION_PZONE,POS_FACEUP,true)~=0 then
			Duel.RaiseSingleEvent(e:GetHandler(),EVENT_CUSTOM+515004000,e,0,0,0,0)
		end
	end
end
function c515004000.wtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
end
function c515004000.wop(e,tp,eg,ep,ev,re,r,rp)
