...
function c240100214.settg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c240100214.filter,tp,0,LOCATION_MZONE,1,nil) and Duel.IsExistingMatchingCard(c240100214.spfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
...
function c240100214.setop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and Duel.ChangePosition(tc,POS_FACEUP_DEFENSE)~=0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		local g=Duel.SelectMatchingCard(tp,c240100214.spfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
