...
function c210014810.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_POSITION+CATEGORY_TOHAND+CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_SUMMON_SUCCESS)
	e1:SetCost(c210014810.cost)
	e1:SetTarget(c210014810.target)
	e1:SetOperation(c210014810.activate)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e3:SetTarget(c210014810.target2)
	c:RegisterEffect(e3)
end
c210014810.listed_names={0x9b}
function c210014810.mfilter(c)
...
function c210014810.gfilter(c,ft,e,tp,g)
...
function c210014810.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(c210014810.cfilter,tp,LOCATION_ONFIELD+LOCATION_HAND+LOCATION_GRAVE,0,1,nil) end
	local cg=Duel.SelectMatchingCard(tp,c210014810.cfilter,tp,LOCATION_ONFIELD+LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil)
	Duel.Remove(cg,POS_FACEUP,REASON_COST)
end
function c210014810.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c210014810.filter,1,nil,tp) end
	local g=eg:Filter(c210014810.filter,nil,tp)
...
function c210014810.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
	if chk==0 then return rp==1-tp and tc:IsOnField() and tc:IsCanChangePosition() end
	Duel.SetTargetCard(tc)
end
function c210014810.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.ChangePosition(g,POS_FACEUP_DEFENSE,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK)~=0 then
		...
		if check and pg and Duel.IsExistingMatchingCard(c210014810.mfilter,tp,LOCATION_MZONE,0,1,nil)
			and Duel.IsExistingMatchingCard(c210014810.gfilter,tp,LOCATION_GRAVE,0,1,nil,ft,e,tp,pg) then
			Duel.Hint(HINT_SELECTMSG,tp,HINT_CARD)
			local sg=Duel.SelectMatchingCard(tp,c210014810.gfilter,tp,LOCATION_GRAVE,0,1,1,nil,ft,e,tp,pg)
