--Unexpected Daiagram
--AlphaKretin
function c210310252.initial_effect(c)
	--Activate
	...
	e4:SetTarget(c210310252.indtg)
	e4:SetValue(c210310252.indct)
	c:RegisterEffect(e4)
	--destroy
...
function c210310252.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(nil,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,e:GetHandler())
		and Duel.IsExistingMatchingCard(c210310252.thfilter,tp,LOCATION_DECK,0,1,nil) end
	local g=Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_ONFIELD,0,e:GetHandler())
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c210310252.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.Destroy(g,REASON_EFFECT)~=0 then
		...
		local g=Duel.SelectMatchingCard(tp,c210310252.thfilter,tp,LOCATION_DECK,0,1,1,nil)
