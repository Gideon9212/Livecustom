...
function c515958930.initial_effect(c)
	...
	e1:SetCondition(c515958930.penconn)
	e1:SetTarget(c515958930.pentgg)
	e1:SetOperation(c515958930.penopp)
	c:RegisterEffect(e1)
	...
	e2:SetCondition(c515958930.pensetcon)
	e2:SetTarget(c515958930.pensettg)
	e2:SetOperation(c515958930.pensetop)
	c:RegisterEffect(e2)
	--Xyz to lvl7
...
--Destruction hand and scale
function c515958930.penconn(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c515958930.penf1,tp,LOCATION_ONFIELD,0,1,nil)
end
function c515958930.penff1(c)
	return c:IsOnField()
end
function c515958930.penff2(c,self)
    if c:IsCode(c,515958930) and c:GetFieldID()==self then 
    return c end
end
function c515958930.pentgg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	local self=e:GetHandler():GetFieldID()
	if chk==0 then return Duel.IsExistingTarget(c515958930.penff2,tp,LOCATION_HAND,0,1,nil,self) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,c515958930.penff2,tp,LOCATION_HAND,0,1,1,nil,self)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g2=Duel.SelectTarget(tp,c515958930.penff1,tp,LOCATION_ONFIELD,0,1,1,nil)
	g1:Merge(g2)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g1,2,0,0)
end
function c515958930.penopp(e,tp,eg,ep,ev,re,r,rp)
...
--Xyz UP materials
function c515958930.filter2(c,e,tp,mc,rk)
...
function c515958930.spxyztg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	and Duel.IsExistingMatchingCard(c515958930.spxyzfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,e:GetHandler()) end
