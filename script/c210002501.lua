...
function c210002501.initial_effect(c)
    ...
    e2:SetCondition(c210002501.indcon)
    e2:SetTarget(c210002501.indtg)
    e2:SetValue(c210002501.indval)
    c:RegisterEffect(e2)
    local e3=e2:Clone()
...
function c210002501.indcon(e)
    return Duel.IsExistingMatchingCard(c210002501.cfilter,tp,LOCATION_ONFIELD,0,1,nil)
end
function c210002501.indtg(e,c)
    return c:IsType(TYPE_CONTINUOUS) and aux.IsCodeListed(c,46986414)
end
function c210002501.indval(e,re,tp)
...
function c210002501.activate(e,tp,eg,ep,ev,re,r,rp)
    ...
    if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(210002501,0)) then
