...
--AlphaKretin
function c515310156.initial_effect(c)
	--xyz summon
	...
	e1:SetValue(c515310156.efilter)
	c:RegisterEffect(e1)
	--pierce
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCondition(c515310156.condition)
	e2:SetCode(EFFECT_PIERCE)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e3:SetCode(EVENT_PRE_BATTLE_DAMAGE)
	e3:SetCondition(c515310156.damcon)
	e3:SetOperation(c515310156.damop)
	c:RegisterEffect(e3)
end
function c515310156.efilter(e,te)
	return te:IsActiveType(TYPE_TRAP) and te:GetOwner()~=e:GetOwner()
end
function c515310156.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetOverlayGroup():IsExists(Card.IsCode,1,nil,91949988)
end
function c515310156.damcon(e,tp,eg,ep,ev,re,r,rp)
...
function c515310156.damop(e,tp,eg,ep,ev,re,r,rp)
