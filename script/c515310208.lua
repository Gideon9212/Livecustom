...
--AlphaKretin
function c515310208.initial_effect(c)
	--Activate
	...
	e1:SetTarget(c515310208.target)
	e1:SetOperation(c515310208.activate)
	c:RegisterEffect(e1)
	...
	e2:SetTarget(c515310208.indestg)
	e2:SetValue(1)
	c:RegisterEffect(e2)
	--Effect on summon
	...
	e3:SetCountLimit(1)
	e3:SetCondition(c515310208.effcon)
	e3:SetTarget(c515310208.postg)
	e3:SetOperation(c515310208.posop)
	e3:SetLabel(TYPE_FUSION)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	e4:SetCategory(CATEGORY_ATKCHANGE)
	e4:SetTarget(c515310208.atktg)
	e4:SetOperation(c515310208.atkop)
	e4:SetLabel(TYPE_SYNCHRO)
	c:RegisterEffect(e4)
	local e5=e3:Clone()
	e5:SetCategory(0)
	e5:SetTarget(c515310208.atttg)
	e5:SetOperation(c515310208.attop)
	e5:SetLabel(TYPE_XYZ)
	c:RegisterEffect(e5)
	local e6=e3:Clone()
	e6:SetCategory(0)
	e6:SetTarget(c515310208.mvtg)
	e6:SetOperation(c515310208.mvop)
	e6:SetLabel(TYPE_LINK)
	c:RegisterEffect(e6)

end
function c515310208.filter(c)
	return c:IsSetCard(0xf35) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c515310208.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c515310208.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c515310208.activate(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c515310208.filter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,g)
	end
end
function c515310208.indestg(e,c)
	return c:IsSummonType(SUMMON_TYPE_SPECIAL) and c:IsPreviousLocation(LOCATION_GRAVE) and c:IsSetCard(0x1f35)
end
function c515310208.effcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:GetCount()==1 and eg:GetFirst():IsType(e:GetLabel()) and eg:GetFirst():IsSetCard(0x2f35)
end
function c515310208.postg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c515310208.posop(e,tp,eg,ep,ev,re,r,rp)
...
function c515310208.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c515310208.atkop(e,tp,eg,ep,ev,re,r,rp)
...
function c515310208.atttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_GRAVE,0,1,nil) end
end
function c515310208.attop(e,tp,eg,ep,ev,re,r,rp)
...
function c515310208.zonechk(zone,tp)
	local val=0
	for i=0,4 do
...
function c515310208.mvfilter(c,lc)
	return not lc:GetLinkedGroup():IsContains(c)
end
function c515310208.mvtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=eg:GetFirst()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and not chkc:IsControler(tp) and c515310208.mvfilter(chkc,c) end
	if chk==0 then return Duel.IsExistingMatchingCard(c515310208.mvfilter,tp,0,LOCATION_MZONE,1,nil,c) and c515310208.zonechk(c:GetLinkedZone(),tp) end
end
function c515310208.mvop(e,tp,eg,ep,ev,re,r,rp)
	local c=eg:GetFirst()
	if not e:GetHandler():IsRelateToEffect(e) or not Duel.IsExistingMatchingCard(c515310208.mvfilter,tp,0,LOCATION_MZONE,1,nil,c) then return end
