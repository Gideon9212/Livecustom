	...
	if Duel.SpecialSummonStep(bc,0,tp,1-tp,false,false,POS_FACEUP_ATTACK) then
			...
			if c:IsFaceup() and c:IsRelateToEffect(e) then
				if bc and bc:IsFaceup() and c:IsFaceup() and c:IsRelateToEffect(e) then
					...
					e3:SetCondition(c5154242594.con)
					e3:SetTarget(c5154242594.attacktarget)
					e3:SetLabelObject(bc)
					e3:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
					e3:SetValue(1)
					c:RegisterEffect(e3)
					bc:RegisterFlagEffect(5154242594,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
				end
			end
	end
end
function c5154242594.attacktarget(e,c)
...
function c5154242594.actlimit(e,c)
