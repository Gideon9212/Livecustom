...
function c515242586.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(515242586,2))
	e3:SetCountLimit(1,5152425863)
...
function c515242586.filter1(c,e,tp)
	...
	return c:IsFaceup() and c:IsSetCard(0x666) and c:IsAbleToDeckAsCost()
		and Duel.IsExistingMatchingCard(c515242586.filterz,tp,LOCATION_DECK,0,1,nil,code,e,tp)
end
function c515242586.filterz(c,code,e,tp)
...
if not e:GetHandler():IsRelateToEffect(e) then return end
	...
	local g=Duel.SelectMatchingCard(tp,c515242586.filterz,tp,LOCATION_DECK,0,1,1,nil,code,e,tp)
	if g:GetCount()>0 then
