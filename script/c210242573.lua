...
function c210242573.initial_effect(c)
	...
	e3:SetCondition(c210242573.standcon)
	e3:SetTarget(c210242573.standtg)
	e3:SetCountLimit(1)
	e3:SetOperation(c210242573.standop)
	c:RegisterEffect(e3)
	--Shuffle
	...
	e4:SetCost(c210242573.shfcost)
	e4:SetTarget(c210242573.shftg)
	e4:SetCountLimit(1)
	e4:SetOperation(c210242573.shfop)
	c:RegisterEffect(e4)
	--Pend
...
function c210242573.tgcon(e)
	...
	local g=Duel.GetMatchingGroup(c210242573.codefilter,tp,LOCATION_EXTRA,0,nil)
...
function c210242573.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(4066,2)) then
		...
		if sg:GetFirst():GetCode()==210242564 then
...
function c210242573.standcon(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c210242573.codefilter,tp,LOCATION_EXTRA,0,nil)
	return g:GetClassCount(Card.GetCode)>=2
end
function c210242573.standfilter(c)
	return c:IsSetCard(0x666) and c:IsFaceup() and c:IsType(TYPE_MONSTER)
end
function c210242573.standtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE,LOCATION_MZONE) and chkc:IsFaceup() end
	if chk==0 then return Duel.IsExistingTarget(c210242573.standfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c210242573.standfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
function c210242573.standop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
...
function c210242573.shfcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(c210242573.codefilter,tp,LOCATION_EXTRA,0,nil)
	if chk==0 then return g:GetClassCount(Card.GetCode)>=3 end
end
function c210242573.shffilter(c)
	return c:IsType(TYPE_PENDULUM) and c:IsSetCard(0x666) and c:IsFaceup() and c:IsAbleToDeck()
end
function c210242573.shftg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,nil)
		and Duel.IsExistingMatchingCard(c210242573.shffilter,tp,LOCATION_EXTRA,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c210242573.shfop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local g=Duel.GetMatchingGroup(c210242573.shffilter,tp,LOCATION_EXTRA,0,nil)
...
function c210242573.pendcon(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c210242573.codefilter,tp,LOCATION_EXTRA,0,nil)
	return eg:IsExists(c210242573.pendfilter,eg:GetCount(),nil,tp) and g:GetClassCount(Card.GetCode)>=4
end
