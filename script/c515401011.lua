...
function c515401011.initial_effect(c)
	...
	e1:SetTarget(c515401011.target)
	e1:SetOperation(c515401011.operation)
	c:RegisterEffect(e1)
end
function c515401011.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:GetBaseAttack()>0 and c:IsSetCard(0xf18)
end
function c515401011.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and c515401011.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c515401011.filter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectTarget(tp,c515401011.filter,tp,LOCATION_REMOVED,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,g,g:GetCount(),0,0)
end
function c515401011.operation(e,tp,eg,ep,ev,re,r,rp)
