...
function c210242594.initial_effect(c)
	--link summon
	...
	e1:SetValue(c210242594.atkval)
	c:RegisterEffect(e1)
	--chain attack
	local e2=Effect.CreateEffect(c)
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e2:SetCountLimit(1,210242589)
	e2:SetCode(EVENT_BATTLE_DESTROYING)
	e2:SetCondition(c210242594.atcon)
	e2:SetTarget(c210242594.attg)
	e2:SetOperation(c210242594.atop)
	c:RegisterEffect(e2)
end
function c210242594.atcon(e,tp,eg,ep,ev,re,r,rp)
...
function c210242594.attg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c210242594.atop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	if Duel.SpecialSummonStep(bc,0,tp,1-tp,false,false,POS_FACEUP_ATTACK) then
		...
		if c:IsFaceup() and c:IsRelateToEffect(e) then
			if bc and bc:IsFaceup() and c:IsFaceup() and c:IsRelateToEffect(e) then
				...
				e2:SetCondition(c210242594.con)
				e2:SetValue(1)
				...
				e3:SetCondition(c210242594.con)
				e3:SetTarget(c210242594.attacktarget)
				e3:SetLabelObject(bc)
				e3:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
				e3:SetValue(1)
				c:RegisterEffect(e3)
				bc:RegisterFlagEffect(210242594,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
			end
		end
	end
end
function c210242594.attacktarget(e,c)
...
function c210242594.actlimit(e,c)
	return not c==e:GetLabelObject()
end
function c210242594.con(e,tp,eg,ep,ev,re,r,rp)
	local bc=e:GetLabelObject()
	return bc:GetFlagEffect(210242594)>0
end
function c210242594.atkval(e,c)
