...
c210002507.card_code_list={46986414,210002507}
...
function c210002507.tgfilter(c,e,tp)
	return (c:IsFaceup() or c:IsLocation(LOCATION_HAND)) and c:IsCanBeFusionMaterial()
		...
		and Duel.IsExistingMatchingCard(c210002507.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c:GetCode())
end
function c210002507.spfilter(c,e,tp,code)
...
function c210002507.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,c210002507.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,g:GetFirst():GetCode())
